...
export class MainPage {
    ...
    async getData() {
        try {
            const { data } = await ajax.get(videocardUrls.getVideocards());
    ...
    async addCard() {
        ...
        try {
            await ajax.post(videocardUrls.createVideocard(), newCard);
            await this.getData();
        } catch (error) {
    ...
    async deleteCardById() {
        ...
        try {
            const { status } = await ajax.delete(videocardUrls.removeVideocardById(idToDelete));
            if (status === 200) {
                await this.getData();
            } else {
    ...
    filterByViews(min, max) {
        ajax.get(videocardUrls.getVideocardsFiltered(min, max))
            .then(({ data }) => {
                this.renderData(data);
            })
            .catch(error => console.error('Ошибка при фильтрации с сервера:', error));
    }
